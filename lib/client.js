/**
 * Browser bundle of theme-plugin (static client plugin).
 ...
 *  - 上传自定义 JS 主题（__ModuleLoader__.load 格式，overrideTokens/CSS/slots）
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── 持久化：Host 磁盘（webServer 路由 /api/theme-plugin，换浏览器不丢）──
    ...
    function saveStored(themes, selected) {
      fetch("/api/theme-plugin", {
        method: "POST",
    ...
    function apply(ctx) {
      ...
      function executeThemeSource(source) {
        ...
        var trackedHead = new Proxy(document.head, {
            ...
            if (prop === "appendChild" || prop === "append") {
              return function (node) {
                if (node && node.tagName === "STYLE") {
                  try {
                    if (captured && typeof captured.id === "string") node.dataset.dynTheme = captured.id;
                  } catch (err) {}
                  trackedTags.push(node);
                }
                return Reflect.apply(value, target, [node]);
              };
            }
        ...
        var trackedDocument = new Proxy(document, {
          get: function (target, prop) {
            if (prop === "head") return trackedHead;
            var value = Reflect.get(target, prop, target);
            if (typeof value !== "function") return value;
            return function () {
              return Reflect.apply(value, target, arguments);
        ...
        run(fakeWindow, trackedDocument, console);
      ...
      function restoreUploaded() {
        loadStored(function (stored) {
          ...
          if (last) {
            try {
              applyJsTheme({ id: last.id, name: last.name, source: last.source, active: true, disposers: [], tags: [] });
            } catch (err) {
      ...
      function ThemePage() {
        ...
        function switchJsTheme(id) {
          ...
          try {
            applyJsTheme({ id: entry.id, name: entry.name, source: entry.source, active: true, disposers: [], tags: [] });
        ...
        function onFile(e) {
          ...
          if (file.size > 200000) {
            setMessage({ type: "error", text: "文件过大（上限 200KB）" });
            return;
          }
          ...
          reader.onload = function () {
            ...
            try {
              ...
              applyJsTheme(entry);
              var next = entries.concat([{ id: id, name: id, source: source, active: true }]);
              setEntries(next);
        ...
        function remove(id) {
          ...
          if (jsOrder.length > 0) {
            ...
            var last = jsThemes.get(lastId);
            if (last) {
              try { applyJsTheme(last); } catch (err) {}
            }
          }
          persist(next);
        }
